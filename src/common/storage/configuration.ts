...
/**
 * Записи через эту функцию идут по очереди: `accounts` — один ключ localforage,
 ...
 *
 * Очередь покрывает только этот путь. Страница настроек при добавлении и удалении аккаунта пишет
 * весь массив через `updateConfiguration` мимо неё — там это безопасно, потому что она перечитывает
 * хранилище непосредственно перед записью.
 *
